...
const CTDashboard3 = (props) => {
  ...
  useEffect(() => {
    axios
      .get(`${API_URL}/dashboard3_data.php${props.location.search}`)
  ...
  if (pageData.DEFECTDESCRIPTION && pageData.DEFECTDESCRIPTION.DEFECT) {
    ...
    if (ProDATA.length > 0) {
      ProDATA.forEach(function (item, index) {
        if (item.SERIES === "Pro3000") {
          Pro3000Data.push(item);
        } else if (item.SERIES === "Pro6000") {
          Pro6000Data.push(item);
        } else if (item.SERIES === "Pro8000") {
          Pro8000Data.push(item);
        }
      });
    }
  }
  console.table(Pro3000Data);
